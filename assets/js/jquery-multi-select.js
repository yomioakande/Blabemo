(function ($) {
  ...
  multiSelect.prototype.bindEvent = function () {
    ...
    this.REAL_IPT_EL[0].addEventListener(
      "compositionend",
      this.iptCompositionEnd.bind(this),
      false
    );
  };
  ...
  multiSelect.prototype.renderOptions = function () {
    this.optionLists.forEach((item) => {
      var option = this.renderOptionItem(
        item,
        (function (item, multiSelectObj) {
          return function () {
  ...
  function selectOption(item, multiSelectObj) {
    ...
    for (var i = 0; i < multiSelectObj.optionLists.length; i++) {
      if (
        multiSelectObj.optionLists[i][multiSelectObj._ORDER_ATTR] ==
        item[multiSelectObj._ORDER_ATTR]
      ) {
        removedIndex = i;
      }
    }

    multiSelectObj.renderTags(
      multiSelectObj.optionLists.splice(removedIndex, 1)[0]
    );
  ...
  multiSelect.prototype.renderTags = function (obj) {
    ...
    $(tagDel).bind("click", (e) => {
      ...
      if (
        targetClassName.includes("tag-txt") ||
        targetClassName.includes("tag-del")
      ) {
        target = targetClassName.includes("tag-txt")
          ? target
          : target.parentNode;
        ...
        while (i < this.optionLists.length) {
          if (
            Number(this.optionLists[i][this._ORDER_ATTR]) >=
        ...
        if (insertedPos == this.optionLists.length) {
          ...
          var tailArr = this.optionLists.slice(
            insertedPos,
  ...
  multiSelect.prototype.search = function (ipt) {
    var domLists = this.getEl("option"),
      noneNumber = 0;
  ...
  $.fn.extend({
    ...
    multiSelect: function (options = {}) {
      ...
      if (!Array.isArray(options.data)) {
        throw new Error(
          "arguments[0] must have the property of 'data'! 参数中必须包含data属性，{text:'选项名',value:'顺序名，必须为数值型'}（其他属性为您需要的参数）"
        );
      }
